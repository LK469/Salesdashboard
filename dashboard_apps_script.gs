...
/** Prefer Cerby_SF_Account_Import, then gid 1045088599, then generic Account tabs. */
function findAccountSheet_(spreadsheet, activeSs) {
  ...
  for (const book of books) {
    ...
    // Case-insensitive tab name match
...
function getIcpAccountData(ss, oppMetrics) {
  ...
  // Column layout from Cerby_SF_Account_Import tab in COEFF_SS_ID
  const C = {
    id              : col('id'),
    name            : col('name'),
    website         : col('website'),
    ...
    // Legacy / optional fields on other Account exports
